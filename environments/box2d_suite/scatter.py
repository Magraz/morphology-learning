...
class ScatterEnv(gym.Env):
    ...
    def _init_agents(self):
        ...
        spawn_radius = 2.0
        angles = np.random.uniform(0, 2 * np.pi, self.n_agents)
        radii = spawn_radius * np.sqrt(np.random.uniform(0, 1, self.n_agents))
        positions = [
            (cx + r * np.cos(a), cy + r * np.sin(a)) for r, a in zip(radii, angles)
        ]
    ...
    def _calculate_reward(self):
        ...
        # Pairwise distance matrix (n_agents, n_agents)
        diff = positions[:, np.newaxis, :] - positions[np.newaxis, :, :]
        dists = np.linalg.norm(diff, axis=-1)
        np.fill_diagonal(dists, np.inf)

        # Mean of each agent's nearest-neighbor distance, normalized by world diagonal
        min_neighbor_dists = dists.min(axis=1)
        potential = min_neighbor_dists.mean() / self.world_diagonal

        # Shaping reward: positive when agents spread out
        reward = 0.0
        if hasattr(self, "_prev_scatter_potential"):
            reward = (potential - self._prev_scatter_potential) * 10.0
            if abs(reward) < 1e-5:
                reward = 0.0
        self._prev_scatter_potential = potential
    ...
    def reset(self, seed=None, options=None):
        ...
        # Reset scatter reward potential
        if hasattr(self, "_prev_scatter_potential"):
            del self._prev_scatter_potential
    ...
    def step(self, actions):
        ...
        if self.contact_listener.boundary_collision:
            ...
            task_reward, terminated = self._get_rewards()
