...
def _scatter_add(src, index, dim, dim_size):
    """Native-torch replacement for ``torch_scatter.scatter_add``.

    Sums ``src`` into a zero tensor along ``dim`` at the positions given by the
    1-D ``index`` (which indexes into ``dim``), producing an output whose size
    along ``dim`` is ``dim_size``. Matches the semantics DCG relied on without
    the compiled ``torch_scatter`` dependency.
    """
...
class DeepCoordinationGraphMAC(BasicMAC):
    ...
    def greedy(self, f_i, f_ij, available_actions=None):
        ...
        if len(self.edges_from) > 0 and self.iterations > 0:
            ...
            for iteration in range(self.iterations):
                ...
                msg = _scatter_add(src=messages[0], index=self.edges_to, dim=1, dim_size=self.n_agents)
                msg += _scatter_add(src=messages[1], index=self.edges_from, dim=1, dim_size=self.n_agents)
                utils = f_i + msg
    ...
    def _set_edges(self, edge_list):
        ...
        self.edges_n_in = _scatter_add(src=self.edges_to.new_ones(len(self.edges_to)),
                                        index=self.edges_to, dim=0, dim_size=self.n_agents) \
                          + _scatter_add(src=self.edges_to.new_ones(len(self.edges_to)),
